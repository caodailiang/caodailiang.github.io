...
比如因为X86的CPU都是小端所以使用 bpf_bpfel.o 和 bpf_bpfel.go 即可，最终与 main.go 一起编译就形成了ebpf程序。ebpf程序的字节码 bpf_bpfel.o 会以二进制数据的形式内嵌到这两个go源文件中，具体说就是利用 `go:embed` 特性嵌到 `_BpfBytes` 变量中。
...
#### 操作实践
以 kprobe 为例，统计系统调用 sys_execve 被调用的次数，使用 `map` 传递数据。在 examples 目录下新建一个 kprobe_count 示例，C源码：
```cgo
#include "common.h"
...
使用 `bpf2go` 根据上面的C代码生成bpf字节码和Go代码。
