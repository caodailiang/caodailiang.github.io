...
我们可以看一下tracepoint_in_c例子，其中包含以下几个文件：
```
...
1 directory, 6 files
```
其中C语言文件tracepoint.c就是ebpf程序内核态部分，bpf_bpfeb.o和bpf_bpfel.o是编译后的bpf字节码文件，bpf_bpfel.go/bpf_bpfeb.go则是bpf2go基于字节码生成，提供给用户态程序main.go使用。

bpf_bpfeb.o和bpf_bpfel.o分别对应大小端不同的CPU架构，后缀_bpfeb中的eb代表Big Ending，同理el代表Little Ending。bpf_bpfel.go/bpf_bpfeb.go文件也是如此，所以最终根据具体的CPU架构只会使用二中其一。

比如因为X86的CPU都是小端所以使用bpf_bpfel.o和bpf_bpfel.go即可，最终与main.go一起编译就形成了ebpf程序。ebpf程序的字节码bpf_bpfel.o会以二进制数据的形式内嵌到这两个go源文件中，具体说就是利用go:embed特性嵌到_BpfBytes变量中。
...
var _BpfBytes []byte
```
bpf_bpfel.go文件中也包含一个重要的结构 `bpfObjects`，对应eBPF程序的两个关键组成：eBPF程序数据和用于用户态和内核态数据交互的map。eBPF程序还有一个关键组成就是挂载点（attach point）。
```
...
#### 操作实践
以kprobe为例，统计系统调用sys_execve被调用的次数，使用 `map` 传递数据。在example目录下新建一个kprobe_count示例，C源码：
```
#include "common.h"
...
```
其中第一行的 `common.h` 是cilium/ebpf提供的开发ebpf程序用户态部分所需的头文件，在 `example/headers`目录中。 
```
$ ls kprobe_count/
kprobe.c
```
...
上述第1步中的bpf2go可由Go语言工具链提供的go generate工具来驱动，在main.go的main函数上面增加一行go:generate指示语句，语句内容即为之前的bpf2go命令。当我们执行go generate语句时，go generate会扫描到该指示语句，并执行后面的命令。
```
...
...
```
go generate指示语句可以使用变量参数，变量的值在Makefile中定义。
